...
import torch
import uvicorn
from chatglm_utils import generate_chatglm3, generate_stream_chatglm3, process_response
...
if __name__ == "__main__":
    ...
    config = get_settings()
    chatglm_path = config.CHATGLM_PATH
    tokenizer = AutoTokenizer.from_pretrained(chatglm_path, trust_remote_code=True)
    #model = AutoModel.from_pretrained(chatglm_path, trust_remote_code=True).cuda
    model = AutoModel.from_pretrained(chatglm_path, trust_remote_code=True).float() #CPU
